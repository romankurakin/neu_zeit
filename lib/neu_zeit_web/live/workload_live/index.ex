defmodule NeuZeitWeb.WorkloadLive.Index do
  ...
  def render(assigns) do
    ~H"""
    <Layouts.app flash={@flash} nav={Nav.sections(@term)} current_path={~p"/terms/#{@term}/workload"} terms={@terms} current_term={@term}>
      ...
      <div class={["grid gap-4", @form && "lg:grid-cols-[3fr_2fr]"]}>
        <div class="min-w-0">
          <.table id="workload" rows={@rows} row_id={&"workload-#{&1.id}"} empty_message={gettext("Add subjects, teachers and groups to the teaching load.")}>
            <:col :let={row} label={gettext("Course")}>
              <.link navigate={Nav.with_return(~p"/courses/#{row.session.course_component.course_id}", ~p"/terms/#{@term}/workload")} class="link">{row.session.course_component.course.code}</.link>
              <div>{component_kind_label(row.session.course_component.kind)}</div>
            </:col>
            <:col :let={row} label={gettext("Teacher")} class="whitespace-nowrap">{row.session.teacher.name}</:col>
            <:col :let={row} label={gettext("Groups")} class="whitespace-nowrap">{Enum.map_join(row.session.cohorts, ", ", & &1.name)}</:col>
            <:col :let={row} label={gettext("Academic hours")} numeric>{Workload.hours(row, @term.academic_hour_minutes)}</:col>
            <:col :let={row} label={gettext("Consecutive time slots")} numeric>{row.session.duration_slots}</:col>
            <:col :let={row} label={gettext("Weeks")} class="whitespace-nowrap"><span :if={row.session.automatic_weeks && length(row.session.week_mask) == @term.weeks_count}>{gettext("Automatic")}</span>
              <.teaching_weeks :if={!row.session.automatic_weeks || length(row.session.week_mask) != @term.weeks_count} weeks={row.session.week_mask} total={@term.weeks_count} compact /></:col>
            <:col :let={row} label={gettext("Time profile")} class="whitespace-nowrap">{if row.session.slot_profile, do: slot_profile_label(row.session.slot_profile), else: gettext("No restriction")}</:col>
            <:action :let={row}>
              <.link patch={~p"/terms/#{@term}/workload/#{row.id}/edit"} class="btn btn-ghost">{gettext("Edit")}</.link>
            </:action>
          </.table>
          <.link navigate={~p"/terms/#{@term}/sessions"} class="link inline-block mt-4">{gettext("Individual sessions")}</.link>
        </div>
        <.details_panel :if={@form} title={if @editing, do: gettext("Edit teaching load"), else: gettext("Add teaching load")}
          class="order-first lg:order-last" on_close={JS.patch(~p"/terms/#{@term}/workload")}>
          <.form for={@form} id="workload-form" phx-submit="save" class="flex flex-col gap-4">
            <.input field={@form[:course_component_id]} type="select" label={gettext("Teaching type")}
              prompt={gettext("Choose a teaching type")}
              options={Enum.map(@components, &{"#{&1.course.code} #{&1.course.title} / #{component_kind_label(&1.kind)}", &1.id})} />
            <.input field={@form[:teacher_id]} type="select" label={gettext("Teacher")} prompt={gettext("Choose a teacher")}
              options={Enum.map(@teachers, &{&1.name, &1.id})} />
            <.input field={@form[:contact_hours]} type="number" label={gettext("Academic hours per semester (%{minutes} min)", minutes: @term.academic_hour_minutes)} min="0.01" step="any" required />
            <.input field={@form[:duration_slots]} type="number" label={gettext("Consecutive time slots")} min="1" />
            <.input field={@form[:slot_profile_id]} type="select" label={gettext("Time profile")} prompt={gettext("No restriction")}
              options={Enum.map(@profiles, &{slot_profile_label(&1), &1.id})} />
            <details id="workload-week-limit" class="collapse collapse-arrow bg-base-200" open={!@automatic_weeks} phx-mounted={JS.ignore_attributes("open")}>
              <summary class="collapse-title">{if @automatic_weeks, do: gettext("Restrict available weeks"), else: gettext("Fixed teaching weeks")}</summary>
              <div class="collapse-content">
              <.week_selector id="workload-weeks" weeks={@week_mask} total={@term.weeks_count} parity_presets={!@automatic_weeks} />
              <p :for={{message, opts} <- @form[:week_mask].errors} class="text-error">{Errors.translate_validation({message, opts})}</p>
              </div>
            </details>
            <fieldset class="fieldset">
              <legend class="fieldset-legend">{gettext("Groups")}</legend>
              <p>{gettext("For a shared session, select all attending groups.")}</p>
              <.transfer_list id="workload-cohorts"
                available={cohort_items(@cohorts, @cohort_ids, false)} selected={cohort_items(@cohorts, @cohort_ids, true)}
                available_label={gettext("Other groups")} selected_label={gettext("Attending")} />
              <p :for={{message, opts} <- @form[:cohort_ids].errors} class="text-error">{Errors.translate_validation({message, opts})}</p>
            </fieldset>
