defmodule NeuZeitWeb.PlanLive.Reports do
  ...
  def reports(assigns) do
    ...
    <div :if={@tab == "quality"} class="flex flex-col gap-4">
      <.empty_state :if={@quality.rooms == []} title={gettext("No scheduled sessions")}>
        <:actions>
          <.link patch={workspace_path(@workspace, %{"tab" => "board"})} class="btn btn-primary">{gettext(
            "Open timetable"
          )}</.link>
        </:actions>
      </.empty_state>
      ...
      <.card :if={@quality.rooms != []} title={gettext("Rooms")}>
        <.table id="quality-rooms" rows={@quality.rooms} row_id={&"quality-room-#{&1.id}"}>
          <:col :let={row} label={gettext("Room")} class="whitespace-nowrap">
            <.link
              patch={
              ...
              title={gettext("Show in timetable")}
            >{row.name}</.link>
          </:col>
    ...
    <div :if={@tab == "coverage"} class="flex flex-col gap-4">
      ...
      <p class="type-detail font-semibold">
        {@plan.name}, {if @plan.status == "active",
          do: gettext("Published calendar, including active changes"),
          else: gettext("Calendar from the template, without one-off changes")}
      </p>
      <.table
        id="coverage"
        rows={@coverage}
  ...
  defp quality_table(assigns) do
    ...
    <.table id={@id} rows={@rows} row_id={&"#{@id}-#{&1.id}"}>
      <:col :let={row} label={gettext("Name")} class="whitespace-nowrap">
        <.link
          patch={
            workspace_path(@workspace, %{
              "tab" => "board",
      ...
      <:col :let={row} label={gettext("Long days")} numeric class="whitespace-normal">
        {row.long_days}
      </:col>
