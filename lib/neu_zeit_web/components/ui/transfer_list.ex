    ...
    <div id={@id} phx-hook=".Draggable" data-event={@event} class="grid gap-4 sm:grid-cols-2">
      ...
      >
        ...
        <ul data-role={role} class="list min-h-24 p-2">
          <li :for={item <- items} data-id={item.id} class="list-row items-center gap-2 p-2 cursor-grab active:cursor-grabbing">
            <div class="list-col-grow min-w-0">
              <p>{item.label}</p>
              <p :if={item[:hint]}>{item[:hint]}</p>
            </div>
            <button type="button" class="btn" data-toggle-member={item.id} aria-label={if role == "selected", do: gettext("Remove %{name}", name: item.label), else: gettext("Add %{name}", name: item.label)}>
